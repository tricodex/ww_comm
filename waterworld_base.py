...
class WaterworldBase:
    def __init__(
        ...
        self.communication_buffers = [{} for _ in range(self.n_pursuers)]
        self.intent_flags = [0 for _ in range(self.n_pursuers)]

        self.get_spaces()
        self._seed()

    def determine_intent_from_observation(self, obs):
        food_distances = obs[2 * self.n_sensors:3 * self.n_sensors]
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            ...
            # Clear communication buffers after all agents have acted
            self.communication_buffers = [[] for _ in range(self.n_pursuers)]
    ...
    def observe_list(self):
        ...
        for i, pursuer in enumerate(self.pursuers):
            ...
            full_observation = np.concatenate([pursuer_observation, messages])
        
            
            # Debugging prints
            #print(f"Sensor Obs Length: {len(pursuer_observation)}, Messages Length: {len(messages)}, Full Obs Length: {len(full_observation)}, Expected Length: {self.observation_space[0].shape[0]}")
